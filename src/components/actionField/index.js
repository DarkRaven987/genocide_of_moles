...
import './index.css';
import {incrementFailTimes, changeScore, changeGameDifficulty, findMole, changeUpdateTime} from '../../storage/actions'

class actionField extends React.Component{

    checkForMole = (element) => {
        const {storage, changeScore, changeGameDifficulty , incrementFailTimes, findMole, changeUpdateTime } = this.props;
        if(element.isMole){
            changeScore(5);
